...
export function TestResultScreen() {
  ...
  const summary = useMemo(() => {
    ...
    const { correct } = session.isSubmitted ? computeTestScore(session) : { correct: 0 };
        ...
        {!session.isSubmitted ? (
          <ThemedCard style={{ marginTop: 14 }}>
            ...
            <View style={{ flexDirection: "row", gap: 10, marginTop: 14 }}>
              <PrimaryButton label="Continue Test" onPress={() => navigation.navigate("TakeTest")} style={{ flex: 1 }} />
              <PrimaryButton
                label="Back"
                variant="outline"
