...
function computeScore(session: any): { correct: number } {
  const answers = session?.answersByQuestionId ?? {};
  const questions = session?.questions ?? [];
  let correct = 0;
  for (const q of questions) {
    const chosen = answers[q.id];
    if (typeof chosen === "number" && chosen === q.correctIndex) correct++;
  }
  return { correct };
}
...
export function PracticeResultScreen() {
  ...
  const summary = useMemo(() => {
    ...
    const { correct } = session.isSubmitted ? computeScore(session) : { correct: 0 };
    ...
    return (
      <ScreenContainer edges={["left", "right", "bottom"]}>
        ...
        <ThemedCard style={{ marginTop: 14 }}>
          <ThemedText variant="muted">No practice session found.</ThemedText>
          <PrimaryButton
            label="Open Chapters"
        ...
        {!session.isSubmitted ? (
          <ThemedCard style={{ marginTop: 14 }}>
            ...
            <ThemedText variant="muted" style={{ marginTop: 6, lineHeight: 22 }}>
              You can continue, change answers, and submit when ready.
            </ThemedText>
            ...
            <View style={{ flexDirection: "row", gap: 10, marginTop: 14 }}>
              <PrimaryButton label="Continue" onPress={() => navigation.navigate("PracticeTake")} style={{ flex: 1 }} />
              <PrimaryButton
                label="Exit"
                variant="outline"
                onPress={() => {
                  clearPracticeSession();
                  navigation.navigate("PracticeChapters");
                }}
                style={{ width: 110 }}
              />
            </View>
          ...
          <ThemedCard style={{ marginTop: 14 }}>
            ...
            <View style={{ flexDirection: "row", gap: 10, marginTop: 14 }}>
              <PrimaryButton
                label="Review"
                variant="outline"
                onPress={() => navigation.navigate("PracticeTake")}
                style={{ flex: 1 }}
              />
              <PrimaryButton
                label="Done"
            ...
            filteredRows.map((r) => {
              ...
              return (
                ...
                >
                  ...
                  <ThemedText variant="muted" style={{ marginTop: 6 }}>
                    Tap to jump to this question.{r.isMarked ? "  • Marked" : ""}
                  </ThemedText>
                </Pressable>
              );
            })
